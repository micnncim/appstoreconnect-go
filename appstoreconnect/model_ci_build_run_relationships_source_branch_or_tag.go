/*
App Store Connect API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 3.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package appstoreconnect

import (
	"encoding/json"
)

// checks if the CiBuildRunRelationshipsSourceBranchOrTag type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CiBuildRunRelationshipsSourceBranchOrTag{}

// CiBuildRunRelationshipsSourceBranchOrTag struct for CiBuildRunRelationshipsSourceBranchOrTag
type CiBuildRunRelationshipsSourceBranchOrTag struct {
	Links *AppAvailabilityV2RelationshipsTerritoryAvailabilitiesLinks `json:"links,omitempty"`
	Data  *CiBuildRunRelationshipsSourceBranchOrTagData               `json:"data,omitempty"`
}

// NewCiBuildRunRelationshipsSourceBranchOrTag instantiates a new CiBuildRunRelationshipsSourceBranchOrTag object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCiBuildRunRelationshipsSourceBranchOrTag() *CiBuildRunRelationshipsSourceBranchOrTag {
	this := CiBuildRunRelationshipsSourceBranchOrTag{}
	return &this
}

// NewCiBuildRunRelationshipsSourceBranchOrTagWithDefaults instantiates a new CiBuildRunRelationshipsSourceBranchOrTag object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCiBuildRunRelationshipsSourceBranchOrTagWithDefaults() *CiBuildRunRelationshipsSourceBranchOrTag {
	this := CiBuildRunRelationshipsSourceBranchOrTag{}
	return &this
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *CiBuildRunRelationshipsSourceBranchOrTag) GetLinks() AppAvailabilityV2RelationshipsTerritoryAvailabilitiesLinks {
	if o == nil || IsNil(o.Links) {
		var ret AppAvailabilityV2RelationshipsTerritoryAvailabilitiesLinks
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CiBuildRunRelationshipsSourceBranchOrTag) GetLinksOk() (*AppAvailabilityV2RelationshipsTerritoryAvailabilitiesLinks, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *CiBuildRunRelationshipsSourceBranchOrTag) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given AppAvailabilityV2RelationshipsTerritoryAvailabilitiesLinks and assigns it to the Links field.
func (o *CiBuildRunRelationshipsSourceBranchOrTag) SetLinks(v AppAvailabilityV2RelationshipsTerritoryAvailabilitiesLinks) {
	o.Links = &v
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *CiBuildRunRelationshipsSourceBranchOrTag) GetData() CiBuildRunRelationshipsSourceBranchOrTagData {
	if o == nil || IsNil(o.Data) {
		var ret CiBuildRunRelationshipsSourceBranchOrTagData
		return ret
	}
	return *o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CiBuildRunRelationshipsSourceBranchOrTag) GetDataOk() (*CiBuildRunRelationshipsSourceBranchOrTagData, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *CiBuildRunRelationshipsSourceBranchOrTag) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given CiBuildRunRelationshipsSourceBranchOrTagData and assigns it to the Data field.
func (o *CiBuildRunRelationshipsSourceBranchOrTag) SetData(v CiBuildRunRelationshipsSourceBranchOrTagData) {
	o.Data = &v
}

func (o CiBuildRunRelationshipsSourceBranchOrTag) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CiBuildRunRelationshipsSourceBranchOrTag) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	return toSerialize, nil
}

type NullableCiBuildRunRelationshipsSourceBranchOrTag struct {
	value *CiBuildRunRelationshipsSourceBranchOrTag
	isSet bool
}

func (v NullableCiBuildRunRelationshipsSourceBranchOrTag) Get() *CiBuildRunRelationshipsSourceBranchOrTag {
	return v.value
}

func (v *NullableCiBuildRunRelationshipsSourceBranchOrTag) Set(val *CiBuildRunRelationshipsSourceBranchOrTag) {
	v.value = val
	v.isSet = true
}

func (v NullableCiBuildRunRelationshipsSourceBranchOrTag) IsSet() bool {
	return v.isSet
}

func (v *NullableCiBuildRunRelationshipsSourceBranchOrTag) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCiBuildRunRelationshipsSourceBranchOrTag(val *CiBuildRunRelationshipsSourceBranchOrTag) *NullableCiBuildRunRelationshipsSourceBranchOrTag {
	return &NullableCiBuildRunRelationshipsSourceBranchOrTag{value: val, isSet: true}
}

func (v NullableCiBuildRunRelationshipsSourceBranchOrTag) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCiBuildRunRelationshipsSourceBranchOrTag) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
