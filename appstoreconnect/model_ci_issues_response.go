/*
App Store Connect API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 3.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package appstoreconnect

import (
	"encoding/json"
)

// checks if the CiIssuesResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CiIssuesResponse{}

// CiIssuesResponse struct for CiIssuesResponse
type CiIssuesResponse struct {
	Data  []CiIssue          `json:"data"`
	Links PagedDocumentLinks `json:"links"`
	Meta  *PagingInformation `json:"meta,omitempty"`
}

// NewCiIssuesResponse instantiates a new CiIssuesResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCiIssuesResponse(data []CiIssue, links PagedDocumentLinks) *CiIssuesResponse {
	this := CiIssuesResponse{}
	this.Data = data
	this.Links = links
	return &this
}

// NewCiIssuesResponseWithDefaults instantiates a new CiIssuesResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCiIssuesResponseWithDefaults() *CiIssuesResponse {
	this := CiIssuesResponse{}
	return &this
}

// GetData returns the Data field value
func (o *CiIssuesResponse) GetData() []CiIssue {
	if o == nil {
		var ret []CiIssue
		return ret
	}

	return o.Data
}

// GetDataOk returns a tuple with the Data field value
// and a boolean to check if the value has been set.
func (o *CiIssuesResponse) GetDataOk() ([]CiIssue, bool) {
	if o == nil {
		return nil, false
	}
	return o.Data, true
}

// SetData sets field value
func (o *CiIssuesResponse) SetData(v []CiIssue) {
	o.Data = v
}

// GetLinks returns the Links field value
func (o *CiIssuesResponse) GetLinks() PagedDocumentLinks {
	if o == nil {
		var ret PagedDocumentLinks
		return ret
	}

	return o.Links
}

// GetLinksOk returns a tuple with the Links field value
// and a boolean to check if the value has been set.
func (o *CiIssuesResponse) GetLinksOk() (*PagedDocumentLinks, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Links, true
}

// SetLinks sets field value
func (o *CiIssuesResponse) SetLinks(v PagedDocumentLinks) {
	o.Links = v
}

// GetMeta returns the Meta field value if set, zero value otherwise.
func (o *CiIssuesResponse) GetMeta() PagingInformation {
	if o == nil || IsNil(o.Meta) {
		var ret PagingInformation
		return ret
	}
	return *o.Meta
}

// GetMetaOk returns a tuple with the Meta field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CiIssuesResponse) GetMetaOk() (*PagingInformation, bool) {
	if o == nil || IsNil(o.Meta) {
		return nil, false
	}
	return o.Meta, true
}

// HasMeta returns a boolean if a field has been set.
func (o *CiIssuesResponse) HasMeta() bool {
	if o != nil && !IsNil(o.Meta) {
		return true
	}

	return false
}

// SetMeta gets a reference to the given PagingInformation and assigns it to the Meta field.
func (o *CiIssuesResponse) SetMeta(v PagingInformation) {
	o.Meta = &v
}

func (o CiIssuesResponse) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CiIssuesResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["data"] = o.Data
	toSerialize["links"] = o.Links
	if !IsNil(o.Meta) {
		toSerialize["meta"] = o.Meta
	}
	return toSerialize, nil
}

type NullableCiIssuesResponse struct {
	value *CiIssuesResponse
	isSet bool
}

func (v NullableCiIssuesResponse) Get() *CiIssuesResponse {
	return v.value
}

func (v *NullableCiIssuesResponse) Set(val *CiIssuesResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableCiIssuesResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableCiIssuesResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCiIssuesResponse(val *CiIssuesResponse) *NullableCiIssuesResponse {
	return &NullableCiIssuesResponse{value: val, isSet: true}
}

func (v NullableCiIssuesResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCiIssuesResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
