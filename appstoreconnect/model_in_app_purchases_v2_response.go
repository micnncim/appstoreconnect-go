/*
App Store Connect API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 3.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package appstoreconnect

import (
	"encoding/json"
)

// checks if the InAppPurchasesV2Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &InAppPurchasesV2Response{}

// InAppPurchasesV2Response struct for InAppPurchasesV2Response
type InAppPurchasesV2Response struct {
	Data     []InAppPurchaseV2                       `json:"data"`
	Included []InAppPurchasesV2ResponseIncludedInner `json:"included,omitempty"`
	Links    PagedDocumentLinks                      `json:"links"`
	Meta     *PagingInformation                      `json:"meta,omitempty"`
}

// NewInAppPurchasesV2Response instantiates a new InAppPurchasesV2Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInAppPurchasesV2Response(data []InAppPurchaseV2, links PagedDocumentLinks) *InAppPurchasesV2Response {
	this := InAppPurchasesV2Response{}
	this.Data = data
	this.Links = links
	return &this
}

// NewInAppPurchasesV2ResponseWithDefaults instantiates a new InAppPurchasesV2Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInAppPurchasesV2ResponseWithDefaults() *InAppPurchasesV2Response {
	this := InAppPurchasesV2Response{}
	return &this
}

// GetData returns the Data field value
func (o *InAppPurchasesV2Response) GetData() []InAppPurchaseV2 {
	if o == nil {
		var ret []InAppPurchaseV2
		return ret
	}

	return o.Data
}

// GetDataOk returns a tuple with the Data field value
// and a boolean to check if the value has been set.
func (o *InAppPurchasesV2Response) GetDataOk() ([]InAppPurchaseV2, bool) {
	if o == nil {
		return nil, false
	}
	return o.Data, true
}

// SetData sets field value
func (o *InAppPurchasesV2Response) SetData(v []InAppPurchaseV2) {
	o.Data = v
}

// GetIncluded returns the Included field value if set, zero value otherwise.
func (o *InAppPurchasesV2Response) GetIncluded() []InAppPurchasesV2ResponseIncludedInner {
	if o == nil || IsNil(o.Included) {
		var ret []InAppPurchasesV2ResponseIncludedInner
		return ret
	}
	return o.Included
}

// GetIncludedOk returns a tuple with the Included field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InAppPurchasesV2Response) GetIncludedOk() ([]InAppPurchasesV2ResponseIncludedInner, bool) {
	if o == nil || IsNil(o.Included) {
		return nil, false
	}
	return o.Included, true
}

// HasIncluded returns a boolean if a field has been set.
func (o *InAppPurchasesV2Response) HasIncluded() bool {
	if o != nil && !IsNil(o.Included) {
		return true
	}

	return false
}

// SetIncluded gets a reference to the given []InAppPurchasesV2ResponseIncludedInner and assigns it to the Included field.
func (o *InAppPurchasesV2Response) SetIncluded(v []InAppPurchasesV2ResponseIncludedInner) {
	o.Included = v
}

// GetLinks returns the Links field value
func (o *InAppPurchasesV2Response) GetLinks() PagedDocumentLinks {
	if o == nil {
		var ret PagedDocumentLinks
		return ret
	}

	return o.Links
}

// GetLinksOk returns a tuple with the Links field value
// and a boolean to check if the value has been set.
func (o *InAppPurchasesV2Response) GetLinksOk() (*PagedDocumentLinks, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Links, true
}

// SetLinks sets field value
func (o *InAppPurchasesV2Response) SetLinks(v PagedDocumentLinks) {
	o.Links = v
}

// GetMeta returns the Meta field value if set, zero value otherwise.
func (o *InAppPurchasesV2Response) GetMeta() PagingInformation {
	if o == nil || IsNil(o.Meta) {
		var ret PagingInformation
		return ret
	}
	return *o.Meta
}

// GetMetaOk returns a tuple with the Meta field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InAppPurchasesV2Response) GetMetaOk() (*PagingInformation, bool) {
	if o == nil || IsNil(o.Meta) {
		return nil, false
	}
	return o.Meta, true
}

// HasMeta returns a boolean if a field has been set.
func (o *InAppPurchasesV2Response) HasMeta() bool {
	if o != nil && !IsNil(o.Meta) {
		return true
	}

	return false
}

// SetMeta gets a reference to the given PagingInformation and assigns it to the Meta field.
func (o *InAppPurchasesV2Response) SetMeta(v PagingInformation) {
	o.Meta = &v
}

func (o InAppPurchasesV2Response) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o InAppPurchasesV2Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["data"] = o.Data
	if !IsNil(o.Included) {
		toSerialize["included"] = o.Included
	}
	toSerialize["links"] = o.Links
	if !IsNil(o.Meta) {
		toSerialize["meta"] = o.Meta
	}
	return toSerialize, nil
}

type NullableInAppPurchasesV2Response struct {
	value *InAppPurchasesV2Response
	isSet bool
}

func (v NullableInAppPurchasesV2Response) Get() *InAppPurchasesV2Response {
	return v.value
}

func (v *NullableInAppPurchasesV2Response) Set(val *InAppPurchasesV2Response) {
	v.value = val
	v.isSet = true
}

func (v NullableInAppPurchasesV2Response) IsSet() bool {
	return v.isSet
}

func (v *NullableInAppPurchasesV2Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInAppPurchasesV2Response(val *InAppPurchasesV2Response) *NullableInAppPurchasesV2Response {
	return &NullableInAppPurchasesV2Response{value: val, isSet: true}
}

func (v NullableInAppPurchasesV2Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInAppPurchasesV2Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
